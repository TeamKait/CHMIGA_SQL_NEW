...
>ЗАПРОС 2
```sql
SELECT round(avg(price),1) AS "Средняя цена"
FROM books
...
>ЗАПРОС 6
```sql
SELECT publishing_houses.name, round(avg(price),1) AS "Средняя цена"
FROM books
...
>ЗАПРОС 7
```sql
SELECT publishing_houses.director_surname AS "Фамилия директора издательства", sum(books.circulation) AS "Количество"
FROM books
INNER JOIN publishing_houses ON publishing_houses.id = books.publishing_house_id
GROUP BY books.publishing_house_id
```
...
>ЗАПРОС 8
```sql
SELECT DISTINCT publishing_houses.name AS "Издательство", books.author
FROM books
INNER JOIN publishing_houses ON publishing_houses.id = books.publishing_house_id
```
...
>ЗАПРОС 9
```sql
SELECT publishing_houses.name AS "Издательство", max(books.pages) AS "Количество"
FROM books
INNER JOIN publishing_houses ON publishing_houses.id = books.publishing_house_id
GROUP BY books.publishing_house_id;
```
...
>ЗАПРОС 10
```sql
SELECT publishing_houses.name, count(*) AS "Количество книг"
FROM books
INNER JOIN publishing_houses ON publishing_houses.id = books.publishing_house_id
